...
class RequestFilteringTestCase(TestCase):
    ...
    def _hx_get_requests(self, params=None):
        """Perform HTMX request to requests list endpoint."""
        return self.client.get(reverse('sonar_requests'), params or {}, HTTP_HX_REQUEST='true')

    def test_no_filters_returns_all_requests(self):
        """Test that without filters, all requests are returned"""
        response = self._hx_get_requests()
    ...
    def test_filter_by_verb_get(self):
        ...
        response = self._hx_get_requests({'verb': 'GET'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_verb_post(self):
        ...
        response = self._hx_get_requests({'verb': 'POST'})
        self.assertEqual(response.status_code, 200)
        requests = list(response.context['sonar_requests'])
    ...
    def test_filter_by_path_contains(self):
        ...
        response = self._hx_get_requests({'path': '/api/users'})
        self.assertEqual(response.status_code, 200)
        requests = list(response.context['sonar_requests'])
        self.assertEqual(len(requests), 3)
        for req in requests:
    ...
    def test_filter_by_path_admin(self):
        ...
        response = self._hx_get_requests({'path': '/admin'})
        self.assertEqual(response.status_code, 200)
        requests = list(response.context['sonar_requests'])
        self.assertEqual(len(requests), 1)
    ...
    def test_filter_by_status_200(self):
        ...
        response = self._hx_get_requests({'status': '200'})
        self.assertEqual(response.status_code, 200)
        requests = list(response.context['sonar_requests'])
    ...
    def test_filter_by_status_404(self):
        ...
        response = self._hx_get_requests({'status': '404'})
        self.assertEqual(response.status_code, 200)
        requests = list(response.context['sonar_requests'])
        self.assertEqual(len(requests), 1)
    ...
    def test_pagination_works(self):
        ...
        # Test second page
        response = self._hx_get_requests({'page': 2})
        self.assertEqual(response.status_code, 200)
        page_obj = response.context['page_obj']
